...
CBC and 844 compliant, multi-tenant, **offline-capable**, built for the actual
...
### 2a. Offline-capable operations — the structural gap in the market `[committed]`
...
EduManage's constitution states: *"If EduManage went offline for a week, the
...
- **Bursar-grade financial rigor** — fee edge cases (overpayment, duplicate,
  partial, credit) and receipt audit trails as marketed features, not
  afterthoughts. The bursar is a named human being; money that can't be
  reconciled is trust that can't be rebuilt.
- **Tenant model as a selling point** — every query scoped to `school_id`,
  enforced at middleware. Multi-tenant from day one; school data can never
  leak across schools by design. (Elimikasasa's aggressive switching offers
  target multi-school groups; isolation guarantees matter to them.)
- **22-role RBAC + full audit log** (inherited from Ansai Core) — who did
  what, when, to which record. Auditors and principals ask for this.
...
## 6. One-paragraph sales version

> Kenyan schools don't run on perfect connectivity. EduManage is the school
> management system built for that reality: it keeps working when the network
> doesn't, it speaks Kenyan school language (School DNA, CBC + 844 as
> defaults), it's ready for KNEC compliance out of the box, and it treats
> your bursar's ledger with the rigor real money demands. We migrate you
> from your current system — M-Pesa history included — and run parallel
> until your books agree.
...
3. Offline capability is `[committed]` but unbuilt — the first Labs build
   milestone must be a demonstrable sync story, or the headline collapses.
